...
bool objectComp(Object a, Object b)
{
...
void test() {
		...
		tflite::FlatBufferModel::BuildFromFile("../ssd3.tflite");
    ...
    // TfLiteTensor* scores = nullptr;
		...
		while (true) {
			...
			TfLiteTensor* outputClasses   = interpreter->tensor(interpreter->outputs()[1]);
			auto out_cls = outputClasses->data.f;
			TfLiteTensor* number_of_detection   = interpreter->tensor(interpreter->outputs()[3]);
			auto nums = number_of_detection->data.f;
			for (int i = 0; i < 20; i++){
